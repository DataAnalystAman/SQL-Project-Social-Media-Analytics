...
-- ----------------------- Aggregate Functions ----------------
select max(create_time) from photos_times;select max(create_time) from photos_times;
...
delimiter //
create function generate_gmail(fn text)
returns varchar(50) -- data type of output
deterministic
begin
     -- the value that the function will return
return concat(fn, "@yahoo.com");
end;
//
delimiter ;
select username,
generate_gmail(username) as email
from users;
...
delimiter //
create procedure generatemail
(in fn text) -- data type of output;
deterministic
begin
     -- the value that the function will return
select concat(fn, "@yahoo.com");
end;
//
delimiter ;
call generatemail('amangupta');


delimiter //
create procedure getusername
(in fn int) -- data type of output;
deterministic
begin
     -- the value that the function will return
...
for each row
begin
set new.profile_photo_url = "set profile picture";
set new.bio = "hello guys i am on whatsapp";
set new.email ='add email';
end
//
delimiter ;
...
drop trigger email_trig;

select * from users where user_id = 55;


-- --------------------------------- Indexing ---------------------------
select * from photos;

create index photos_size on photos(size);

select * from photos
where size = '2';

drop index photos_size on photos;
